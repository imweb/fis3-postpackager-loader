...
function uglify(content, conf, file) {
    var ret = UglifyJS.minify(content, conf);
    if (ret.error) {
        console.log('uglify js error', file.id, ret.error);
        return content;
    } else {
        return ret.code;
    }
...
var pack = function (ret, settings) {
    ...
    settings.asyncPacks && loopFiles(function (file) {
        ...
        resourcemapMap[file.id] = buildMap(file.asyncs);
    });
...
// 检测是否忽略压缩
var checkUglifyIgnore = function (file, ignores) {
    var isMatch = false;
    ignores.forEach(function (ignore) {
        if (ignore.test(file.id)) {
            isMatch = true;
        }
    });
    return isMatch;
...
// 打包一个文件及其所有依赖
var _pack = function (file, conf, ret, settings) {
    if (!file) return;
    ...
    var content = file.__deps.concat([file.moduleId]).map(function (id) {
        ...
        // 使用原始内容合并，否则可能重复合并
        if (typeof item.compressContent !== 'undefined') {
            c += item.compressContent;
        } else if (typeof item.__baseContent !== 'undefined') {
            c += item.__baseContent;
        } else if (settings.uglify) {
            if (settings.uglify.ignore && checkUglifyIgnore(item, settings.uglify.ignore)) {
                c += item.getContent();
            } else {
                var result = uglify(item.getContent(), settings.uglify.options || {}, item);
                item.compressContent = result;
                c += result;
            }
        } else {
            c += item.getContent();
    ...
    console.log('cost: ' + cost + 'ms,', ' packTo:', file.id);
...
module.exports = {
    ...
    clear: function (file, resource, ret, settings, opts) {
        ...
        while (content.indexOf(tag) > -1 && nums < 10) {
            content = content.replace(tag, resourcemapMap[file.id]);
            nums++;
        }
        file.setContent(content);
    },
