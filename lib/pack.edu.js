...
var _pack = function (file, conf, ret, settings) {
    ...
    if (file.__deps.length > 0) {
        ...
        if (settings.uglify) {
            try {
                content = uglify(content, file, {
                    compress: {
                        drop_console: true
                    }
                });
            } catch (e) {
                console.error('minify js error', e);
            }
        }
        ...
        console.log('packTo:', file.id, 'cost', cost + 'ms');
    } else if (settings.uglify) {
        try {
            content = file.getContent()
            file.__baseContent = content
            content = uglify(content, file, {
                compress: {
                    drop_console: true
                }
            });
            console.log('uglifyjs', file.id, 'cost', cost + 'ms');
            file.setContent(content);
            file.getHash = function () {
                return fis.util.md5(this.getContent());
            };
